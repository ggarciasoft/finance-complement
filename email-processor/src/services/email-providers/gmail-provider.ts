...
export class GmailProvider implements IEmailProvider {
  ...
  constructor(
    ...
    private emailExtracter: IEmailExtracter,
  ...
  async getEmails(): Promise<EmailList> {
    ...
    const query = `label:recibos-pagos-facturas from:${emailFroms} after:${secondsSinceEpoch(
  ...
  async getEmailDetail(emailId: string): Promise<EmailDetail> {
    ...
    if (res.data.payload) {
      ...
      emailDetail.title =
        res.data.payload.headers?.find((o) => o.name === "Subject")?.value ||
        "";
